...
nuitees_stl_comp %>% 
  filter(dep=="31") %>% 
  select(date, composante_hebdomadaire, jour_sem, we) %>%
...
nuitees_stl_comp %>% 
  ...
  ylab(" composante_hebdomadaire\nDéparts majoritaires<------- | ---> Arrivées majoritaire")+
...
```{r}
#TODO on normalise par departement ( on calcule les max)
#nuites_hebdo_max <- nuitees_stl_comp %>% group_by(dep) %>% summarise(hebdo_max=max(composante_hebdomadaire))

nuitees_stl_comp %>% 
  group_by(dep) %>%     # pour faire des lignes par departement, il faut des groupes
  mutate(composante_hebdomadaire_normalisee = composante_hebdomadaire/max(composante_hebdomadaire)) %>% 
  left_join(nuitees_stlf %>% select(dep,cluster), by="dep") %>% # on rapporte les cluster de k-means calcules plus haut
